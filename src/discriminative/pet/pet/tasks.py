...
class MultiRcProcessor(DataProcessor):
    ...
    def get_train_examples(self, data_dir):
        return self._create_examples(os.path.join(data_dir, "trainfewshot.json"), "train")

    def get_dev_examples(self, data_dir):
        return self._create_examples(os.path.join(data_dir, "devfewshot.json"), "dev")

    def get_test_examples(self, data_dir):
        return self._create_examples(os.path.join(data_dir, "blindtest.json"), "test")
    ...
    def _create_examples(path: str, set_type: str) -> List[InputExample]:
        ...
        with open(path, encoding='utf8') as f:
            ...
            for line in f:
                example_json = json.loads(line)
                passage_idx = passageidx 
                text =  example_json['narrative'].replace('<b>','').replace('</b>','')
                option1 = example_json['option1']
                option2 = example_json['option2']
                
                if example_json['correctanswer']=='option1':
                  option1label = "1"
                  option2label = "0"
                else:
                  option1label = "0"
                ...
                question = 'Based on the previous passage,'
                
                guid = f'p{passage_idx}-q{questionidx}-a{answeridx}'

                meta = {'passage_idx': passage_idx,'question_idx': questionidx,'answer_idx': answeridx,'answer': option1}
                idx = [passage_idx, questionidx, answeridx]
                example1 = InputExample(guid=guid, text_a=text, text_b=question, label=option1label, meta=meta, idx=idx)
                examples.append(example1)
                answeridx = answeridx+1
                
                meta = {'passage_idx': passage_idx,'question_idx': questionidx,'answer_idx': answeridx,'answer': option2}
                idx = [passage_idx, questionidx, answeridx]
                example2 = InputExample(guid=guid, text_a=text, text_b=question, label=option2label, meta=meta, idx=idx)
