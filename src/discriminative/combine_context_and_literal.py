...
def main():
    ...
    context_model = AutoModelForMultipleChoice.from_pretrained(args.context_model_checkpoint)
    context_model.to(device)
    
    # Load the datasets
    logger.info("Loading the context datasets")
    context_dev = MultipleChoiceDataset(
        data_dir=args.context_data_dir, tokenizer=context_tokenizer, task="idiom",
    ...
    dev_gold = [int(json.loads(line)["correctanswer"][len("option"):]) - 1 for line in open(f"{args.context_data_dir}/neurodev.json")]
    ...
    test_gold = [int(json.loads(line)["correctanswer"][len("option"):]) - 1 for line in open(f"{args.context_data_dir}/neurotest.json")]
    ...
    literal_model = AutoModelForMultipleChoice.from_pretrained(args.literal_model_checkpoint)
    literal_model.to(device)
    
    # Load the datasets
    logger.info("Loading the literal datasets")
    literal_dev = MultipleChoiceDataset(
        data_dir=args.literal_data_dir, tokenizer=literal_tokenizer, task="idiom",
        mode=Split.dev, max_seq_length=args.max_seq_len)
    ...
    logger.info(
        f"Best: context = {alpha:.1f}, literal = {1-alpha:.1f}, Dev accuracy: {accuracies[best_index]*100:.1f}")
